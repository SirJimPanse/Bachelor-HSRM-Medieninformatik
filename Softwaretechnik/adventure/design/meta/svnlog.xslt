...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="format-date">
    ...
    <xsl:param name="date" />
    <xsl:param name="part" />
    <xsl:param name="dateSpliter">-</xsl:param>
    <xsl:param name="timeSpliter">T</xsl:param>
    <xsl:param name="format">DMY</xsl:param>

    <xsl:variable name="dateonly" select="substring-before($date, 'T')" />
    <xsl:variable name="year" select="substring-before($dateonly, $dateSpliter)" />
    <xsl:variable name="month" select="substring-before(substring-after($dateonly, $dateSpliter), $dateSpliter)" />
    <xsl:variable name="day" select="substring-after(substring-after($dateonly, $dateSpliter), $dateSpliter)" />
    ...
    <xsl:choose>
      <xsl:when test="$format = 'DMY'">
        <xsl:choose>
          <xsl:when test="$part = 'both'">
            <xsl:value-of select="concat($day, $dateSpliter, $month, $dateSpliter, $year, $timeSpliter, $timeonly)" />
          </xsl:when>
          <xsl:when test="$part = 'date'">
            <xsl:value-of select="concat($day, $dateSpliter, $month, $dateSpliter, $year)" />
          </xsl:when>
          <xsl:otherwise>
      ...
      <xsl:when test="$format = 'YMD'">
        <xsl:choose>
          <xsl:when test="$part = 'both'">
            <xsl:value-of select="concat($year, $dateSpliter, $month, $dateSpliter, $day, $timeSpliter, $timeonly)" />
          </xsl:when>
          <xsl:when test="$part = 'date'">
            <xsl:value-of select="concat($year, $dateSpliter, $month, $dateSpliter, $day)" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="$timeonly" />
          </xsl:otherwise>
