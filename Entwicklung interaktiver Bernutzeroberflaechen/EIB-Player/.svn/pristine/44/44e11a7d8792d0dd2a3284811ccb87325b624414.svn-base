...
import de.hsrm.entintben.mp3_player.gui.MP3PlayerGUI;
...
public class MP3Player extends Observable {
    ...
    public static final String PLAYINGCHANGE = "playing";
    ...
    /**
     * wird aufgerufen wenn sich ,bei am Observer angemeldeten Objekten, etwas ändert.
     */
    public void tell() {
    ...
    public void skipForward() throws JavaLayerException, FileNotFoundException {
        if (isShuffle()) {
            if (isPlaying()) {
                setStopped(true);
                this.stop();
                setActTrack(actPlaylist.getTrack(shuffleValue(0, actPlaylist.getPlaylist().size())));
     ...
     * @return 
     */
    public int shuffleValue(int low, int high) {
     ...
     * @throws FileNotFoundException 
     */
    public Track selectTrack(int number) throws JavaLayerException, FileNotFoundException {
        if (playing) {
            this.stop();
            this.stopped = true;
        }
        ...
        trackNo = number;
        setActTrack(actPlaylist.getTrack(number));
    ...
    public void setActPlaylist(Playlist actPlaylist) {
        Playlist lastActPlaylist = this.actPlaylist;
        this.actPlaylist = actPlaylist;
        changes.firePropertyChange("actPlaylist", lastActPlaylist, actPlaylist);
        tell();
    }
    ...
    /**
     * Setzt die Tracknummer
     * 
     * @param trackNo the trackNo to set
     */
    public void setTrackNo(int trackNo) {
        int lastTrackNo = this.trackNo;
        this.trackNo = trackNo;
        changes.firePropertyChange(ACTTRACKCHANGE, lastTrackNo, trackNo);
    }
    ...
    /**
     *Bekommt Information ob gerade ein Track gespielt wird(true) oder nicht(false).
     * 
     * @param playing the playing to set
     */
    public void setPlaying(boolean playing) {
        boolean lastPlaying = this.playing;
        this.playing = playing;
        changes.firePropertyChange(PLAYINGCHANGE, lastPlaying, playing);
    }
    ...
    /**
     * Bekommt Information ob der Player gestoppt wurde.
     * 
     * @param stopped the stopped to set
     */
    public void setStopped(boolean stopped) {
        boolean lastStopped = this.stopped;
        this.stopped = stopped;
        changes.firePropertyChange("stopped", lastStopped, stopped);
    }
    ...
    /**
     * Bekommt Informtaion ob shuffle eignestellt ist oder nicht.
     * 
     * @param shuffle the shuffle to set
     */
    public void setShuffle(boolean shuffle) {
        boolean lastShuffle = this.shuffle;
        this.shuffle = shuffle;
        changes.firePropertyChange("shuffle", lastShuffle, shuffle);
    }
